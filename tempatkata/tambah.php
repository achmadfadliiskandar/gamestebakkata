...
<body>
    ...
    <div class="content">
        <h1>Tambah Kata</h1>
        <div class="forminput">
        <form action="/tebakkata/tempatkata/tambah.php" method="post">
            <label for="kata">Kata</label>
            <input type="text" name="kata" id="kata" required>
            <input type="submit" id="addkata" name="addkata" value="Save Words">
        </form>
        <?php 
        include '../koneksi.php';
        if(isset($_POST['addkata'])){
            // echo "hello world";
            $kata = trim(htmlspecialchars($_POST['kata']));
            if (empty($kata)) {
                echo "masih kosong";
            }else if (!preg_match("/^[a-zA-Z0-9]{5,}$/", $kata)){
            echo "<script>alert('silahkan Refresh',window.location.assign(''))</script>";
            }else{
                $sql = "INSERT INTO kata(id,kata) VALUES (NULL,'$kata')";
                $query = mysqli_query($connect,$sql);
                if ($query) {
                    echo "data masuk ke dalam db";
                } else {
                    echo "data tidak masuk ke dalam db";
                }
            }
        }
        ?>
